...
// Cek apakah nomeja ada di database
if (isset($_GET["nomeja"])) {
    $nomejax = 0;
    $nomejanya = $_GET["nomeja"];
    $query_meja1 = "SELECT count(*) FROM eat_and_go_meja where nomeja=$nomejanya";
    $result_meja1 = mysqli_query($conn, $query_meja1);
    while ($datameja = mysqli_fetch_array($result_meja1)) {
        $nomejax = $datameja[0];
    }
    if ($nomejax == 0) {
        echo '<!DOCTYPE html>';
        ...
        echo '<h1>Error</h1>';
        echo '<p>NOMER MEJA TIDAK ADA, MOHON SCAN QR CODE LAGI</p>';
        echo '</div>';
        echo '</body>';
        echo '</html>';
        exit;
...
// Cek apakah IDTOKO ada di database
if ($id_toko == 0) {
    echo '<!DOCTYPE html>';
    echo '<html lang="en">';
    echo '<head>';
    ...
    echo '<div class="error-message">';
    echo '<h1>Error</h1>';
    echo '<p>ID TOKO TIDAK DITEMUKAN, MOHON CEK KEMBALI.</p>';
    echo '</div>';
    echo '</body>';
    echo '</html>';
    exit;
...
// Verifikasi IDTOKO ada dalam database
$query_check_toko = "SELECT COUNT(*) as count FROM eat_and_go_toko WHERE IDTOKO = '$id_toko'";
$result_check_toko = mysqli_query($conn, $query_check_toko);
$row_check_toko = mysqli_fetch_assoc($result_check_toko);

if ($row_check_toko['count'] == 0) {
    echo '<!DOCTYPE html>';
    echo '<html lang="en">';
    echo '<head>';
    echo '<link rel="stylesheet" href="style.css">';
...
// Lanjutkan kode lainnya jika IDTOKO valid
$username = $_SESSION['username'] ?? null; // Ambil username pengguna dari sesi, jika ada
$is_admin = ($username === 'admin'); // Cek apakah pengguna adalah admin

// Jika pengguna tidak login, ambil IDTOKO dari URL
if (!$username) {
    if ($id_toko == 0) {
        echo "IDTOKO tidak ditemukan di URL.";
        exit;
    }
} else {
    // Query untuk mengambil data pengguna berdasarkan username
    $query_user = "SELECT * FROM eat_and_go_pengguna WHERE username = '$username'";
    ...
    // Ambil ID Toko yang sesuai dengan pengguna yang login
    $id_toko = $is_admin ? $_POST['id_toko'] : $user_data['IDTOKO'];  // Ambil ID toko dari dropdown atau sesi
}
...
// Query untuk mengambil data barang dengan filter admin
if ($is_admin) {
    // Admin dapat melihat semua barang tanpa filter IDTOKO
    $query_barang = "SELECT * FROM eat_and_go_barang WHERE (namabarang LIKE '%$search_keyword%' OR jenis LIKE '%$search_keyword%')";
} else {
    // Non-admin hanya dapat melihat barang berdasarkan IDTOKO mereka
    $query_barang = "SELECT * FROM eat_and_go_barang WHERE IDTOKO = '$id_toko' AND (namabarang LIKE '%$search_keyword%' OR jenis LIKE '%$search_keyword%')";
}
$result_barang = mysqli_query($conn, $query_barang);

// Hitung total item untuk kategori Makanan
$sql_count_makanan = $is_admin ?
    "SELECT COUNT(*) AS total FROM eat_and_go_barang WHERE jenis = 'Makanan' AND (namabarang LIKE '%$search_keyword%' OR jenis LIKE '%$search_keyword%')" :
    "SELECT COUNT(*) AS total FROM eat_and_go_barang WHERE IDTOKO = '$id_toko' AND jenis = 'Makanan' AND (namabarang LIKE '%$search_keyword%' OR jenis LIKE '%$search_keyword%')";
...
$sql_count_minuman = $is_admin ?
    "SELECT COUNT(*) AS total FROM eat_and_go_barang WHERE jenis = 'Minuman' AND (namabarang LIKE '%$search_keyword%' OR jenis LIKE '%$search_keyword%')" :
    "SELECT COUNT(*) AS total FROM eat_and_go_barang WHERE IDTOKO = '$id_toko' AND jenis = 'Minuman' AND (namabarang LIKE '%$search_keyword%' OR jenis LIKE '%$search_keyword%')";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    mysqli_stmt_bind_param($stmt, "ssssssddi", $nopesanan, $nomeja, $tanggal, $nama, $notelepon, $grandtotal, $jenispembayaran, $terbayar, $id_toko);
    ...
    foreach ($namabarangs as $key => $namabarang) {
        ...
        mysqli_stmt_bind_param($stmt_detail, "ssssddisi", $nopesanan, $kodebarang, $namabarang, $harga, $jumlah, $total, $bungkus_item, $keterangan_item, $id_toko);
    ...
    // echo "<script>window.location.href='print.php?id=$hasilakhir';</script>";
    // exit();
    // Redirect setelah penyimpanan dan kirim IDTOKO ke print.php
    echo "<script>window.location.href='print.php?id=$hasilakhir&id_toko=$id_toko';</script>";
    exit();
...
<head>
    ...
    <style>
        ...
        .filter-buttons button:hover {
            ...
            /* Membesarkan tombol sedikit saat dihover */
        ...
        .cart-buttonn i {
            color: #000;
            /* Warna ikon keranjang */
        }

        #total-items {
            position: absolute;
            top: -8px;
            /* Sesuaikan posisi agar di atas ikon */
            right: -10px;
            /* Sesuaikan posisi agar di sebelah kanan ikon */
            background-color: green;
            /* Warna merah seperti di gambar */
            color: white;
            /* Warna teks putih */
            padding: 2px 6px;
            border-radius: 50%;
        ...
        #cart-total-items {
            position: absolute;
            top: -8px;
            /* Atur posisi agar pas di atas icon keranjang */
            right: -10px;
            /* Atur posisi agar pas di kanan icon keranjang */
            background-color: green;
            color: white;
            padding: 2px 6px;
            border-radius: 50%;
        ...
        .custom-cart-modal-size {
            width: 60%;
            /* Lebar modal default 40% pada layar besar */
            max-height: 80vh;
            /* Batas maksimal tinggi 80% dari viewport */
            margin: auto;
            border-radius: 8px;
            position: fixed;
            top: 10%;
            ...
            overflow-y: auto;
            /* Scroll jika konten terlalu tinggi */
        ...
        .cart-checkout-container {
            width: 100%;
            /* Lebar checkout-container sesuai dengan lebar modal */
            background-color: white;
            padding: 10px;
            display: flex;
            align-items: center;
        ...
        @media (max-width: 768px) {
            .custom-cart-modal-size {
                width: 60%;
                /* Lebar modal 60% pada layar tablet */
                top: 5%;
                left: 50%;
                transform: translateX(-50%);
                max-height: 85vh;
            ...
            .cart-checkout-container {
                ...
                /* Tampilkan dalam kolom pada layar tablet */
                align-items: stretch;
            }

            .cart-checkout-button {
                width: 100%;
                /* Tombol checkout lebar penuh */
                margin-top: 10px;
            }
        }
        ...
        .customm-modal-size {
            width: 90%;
            /* Lebar modal lebih responsif */
            max-width: 400px;
            /* Lebar maksimum modal */
            max-height: 500px;
            /* Tinggi maksimum modal */
            overflow-y: auto;
            /* Mengizinkan scroll jika konten melebihi tinggi modal */
            border-radius: 10px;
        ...
        .modal-item-image {
            ...
            margin-bottom: 10px;
            /* Menambahkan margin di bawah gambar */
        }

        .modal-item-image img {
            border-radius: 8px;
            width: 120px;
            /* Lebar gambar konsisten dengan modal produk */
            height: 120px;
            /* Tinggi gambar konsisten dengan modal produk */
            object-fit: cover;
        ...
        .item-price {
            ...
            /* Ukuran font harga */
            font-weight: bold;
            /* Membuat harga lebih tebal */
            color: #333;
            /* Warna teks */
            margin-bottom: 10px;
            /* Jarak di bawah harga */
        }
        ...
        .modal-actions {
            ...
            padding: 5px;
            /* Padding untuk area tombol */
            position: sticky;
            /* Membuat tombol tetap saat menggulir */
            bottom: 0;
            /* Menempel pada bagian bawah */
            background-color: white;
            /* Warna latar belakang */
            z-index: 100;
            /* Menjaga agar tombol tetap di atas */
            width: 100%;
            /* Lebar penuh */
        }
        ...
        .switch input {
            opacity: 0;
            /* Menghilangkan tampilan input asli */
            width: 0;
            /* Lebar input */
            height: 0;
            /* Tinggi input */
        ...
        input:checked+.slider {
            ...
            /* Warna slider saat aktif */
        }

        input:checked+.slider:before {
            transform: translateX(12px);
            /* Menggerakkan bulatan saat aktif */
        }
        ...
        @media (max-width: 600px) {
            .customm-modal-size {
                ...
                max-width: 350px;
                /* Maximum width untuk perangkat kecil */
            }

            .modal-item-image img {
                max-width: 80px;
                /* Mengurangi ukuran gambar di perangkat kecil */
                max-height: 120px;
                /* Mengurangi ukuran gambar di perangkat kecil */
            }
            ...
            .quantity-controls {
                flex-direction: row;
                /* Mengatur kontrol kuantitas untuk tetap sejajar */
            }
            ...
            .quantity-controls button {
                ...
                padding: 7px;
                /* Padding tombol di perangkat kecil */
            }

            .modal-actions button {
                font-size: 12px;
                /* Mengurangi ukuran font tombol di perangkat kecil */
                padding: 13px;
                /* Mengurangi padding tombol di perangkat kecil */
                border-radius: 20px;
            }
        }
        ...
        .custommm-modal-size {
            ...
            border-radius: 10px;
            justify-content: center;
            /* Mengatur agar isi modal berada di tengah secara vertikal */
            margin: auto;
            /* Margin otomatis untuk memusatkan modal */
        }
        ...
        .modall-item-image img {
            border-radius: 8px;
            width: 100%;
            /* Lebar gambar mengikuti kontainer */
            height: auto;
            /* Tinggi otomatis menjaga proporsi */
            max-width: 150px;
        ...
        .modall-actions {
            ...
            bottom: 0;
            background-color: white;
            /* Warna latar belakang agar konsisten dengan modal */
            z-index: 100;
            /* Pastikan tombol ada di atas konten lainnya */
            width: 100%;
            /* Atur lebar agar tombol mengisi kontainer */
        }
        ...
        @media (max-width: 600px) {
            ...
            .quantity-input {
                width: 25px;
                /* Lebar input kuantitas lebih kecil di perangkat kecil */
            }
        ...
        .w3-section {
            margin-bottom: 16px;
            display: flex;
            /* Menjadikan elemen dalam satu baris */
            align-items: center;
            /* Mengatur agar label dan input sejajar secara vertikal */
        }
        ...
        .w3-section input[readonly] {
            cursor: not-allowed;
            /* Tunjukkan bahwa input tidak dapat diedit */
        }
        ...
        @media (max-width: 600px) {
            .table-container {
                max-height: calc(76.3vh - 50px);
                /* Mengurangi tinggi untuk layar kecil */
                padding: 1px;
                /* Kurangi padding untuk ruang lebih */
                width: 100%;
                /* Memastikan kontainer mengambil seluruh lebar layar */
            }
        }
        ...
        @media (max-width: 400px) {
            .table-container {
                max-height: calc(76.3vh - 50px);
                /* Lebih rendah lagi untuk layar yang sangat kecil */
                padding: 1px;
                /* Padding lebih kecil di layar sangat kecil */
                width: 100%;
                /* Tetap menggunakan seluruh lebar layar */
            }
        }
        ...
        .total-input {
            ...
            font-size: 14px;
            color: green;
            /* Menggunakan warna hijau untuk teks */
            box-sizing: border-box;
            background: transparent;
            /* Menghilangkan background */
        }
        ...
        .button {
            ...
            outline: none;
            /* Menghilangkan outline ketika tombol ditekan */
        }
        ...
        .button.w3-green:hover {
            ...
            transform: scale(1.05);
            /* Membesarkan sedikit tombol saat hover */
            box-shadow: 0 4px 8px rgba(0, 0, 0, 0.2);
            /* Menambahkan shadow saat hover */
        }
        ...
        .button.w3-green:active {
            ...
            /* Shadow lebih kecil saat tombol ditekan */
            transform: scale(1);
            /* Mengembalikan ukuran tombol ke normal saat ditekan */
        }
        ...
        <div class="w3-modal-content w3-card-4 custom-modal-size">
            ...
            <div class="w3-container">
                <div class="table-container">
                    <table class="w3-table-all">
                        <!-- <thead><tr>  <th>Gambar</th>  <th>Nama Barang</th> </tr></thead> -->
                        <tbody id="barangTableBody">
                            <?php while ($row_barang = mysqli_fetch_assoc($result_barang)) { ?>
                                <tr>
                                    <td style="text-align: left;">
                                        <?php if ($row_barang['gambar']): ?>
                                            <img src="data:image/jpeg;base64,<?php echo base64_encode($row_barang['gambar']); ?>"
                                                class="item-image" style="width: 80px; height: auto;" />
                                        <?php else: ?>
                                            <img src="default_image.png" class="item-image"
                                                style="width: 80px; height: auto;" />
                                        <?php endif; ?>
                                    </td>
                                    <td
                                        style="font-size: 14px; text-align: left; word-wrap: break-word; max-width: 180px; overflow: hidden; text-overflow: ellipsis;">
                                        <div class="item-details">
                                            <span
                                                style="color: red; font-weight: bold; display: none;"><?php echo $row_barang['kodebarang']; ?></span>
                                            <span
                                                style="font-weight: bold;"><?php echo $row_barang['namabarang']; ?></span><br>
                                            <span class="item-price"
                                                style="font-size: 16px; font-weight: bold; color: green;"
                                                data-price="<?php echo $row_barang['harga']; ?>">
                                                Rp <?php echo number_format($row_barang['harga'], 2, ',', '.'); ?>
                                            </span>
                                            <span style="color: #000; font-weight: bold; display: none; >Stok: <?php echo $row_barang['stok']; ?></span> <!-- Menampilkan stok -->
                    <span id=" hiddenText"
                                                style="color: #009688; font-weight: bold; display: none;"><?php echo htmlspecialchars($row_barang['jenis']); ?></span><br>
                                        </div>
                                    </td>
                                    <td style="text-align: center;">
                                        <button style="font-weight: bold;" type="button" class="button w3-green" onclick="openProductModal(
            ...
            <div class="checkout-container">
                ...
                <div class="right-options"> <!-- Keranjang Button -->
                    ...
                    <button id="checkout-button" class="checkout-button w3-button w3-green" onclick="closeBarangModal()"
        ...
        <div class="w3-card-4 custom-cart-modal-size">
            ...
            <div class="cart-checkout-container">
                ...
                <div class="cart-right-options">
                    ...
                    <button class="cart-checkout-button w3-button w3-green" id="checkoutButton"
                        href="javascript:void(0);" onclick="closeAllModals();">
                        Bayar
                    </button>
                </div>
            </div>
        ...
        <div class="w3-modal-content w3-card-4 customm-modal-size w3-animate-zoom">
            ...
            <div class="w3-container">
                ...
                <div class="modal-item-details">
                    ...
                    <textarea id="itemKeterangan" placeholder="Tambahkan Catatan (Opsional)"
        ...
        <div class="w3-modal-content w3-card-4 custommm-modal-size w3-animate-zoom">
            ...
            <div class="w3-container">
                ...
                <div class="modall-item-details">
                    ...
                    <textarea id="keteranganInput" placeholder="Tambahkan Catatan (Opsional)"
                        class="w3-input"></textarea>
                </div>
            </div>
            <div class="modall-actions">
                <button class="add-button w3-button w3-green" style="font-weight: bold;" onclick="addToCart()"> <i
            ...
            <div class="w3-container">
                <form id="modalForm" method="POST">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="modalNama" name="nama" placeholder="Nama pemesan"
                            required>
                    </div>

                    <div class="form-group">
                        <label for="modalTelepon">No Telepon:</label>
                        <input type="number" class="form-control" id="modalTelepon" name="notelepon"
                            placeholder="08xxxx" required>
                    </div>
                    <button type="button" class="w3-button w3-green" id="submitModalButton"
                        onclick="submitModalData()">Mulai Pesan</button>
...
<div class="form-group">
    ...
    <input type="number" class="form-control" id="notelepon" name="notelepon" required>
                            ...
                            <th>
                                <div style="display: flex; font-size: 14px;">
                                    <div style="flex: 1; text-align: left;">
                                        <span style="font-size: 16px; color: black; font-weight: bold;">NAMA
                                            BARANG</span><br><span
                                            style="font-size: 16px; color: green; font-weight: bold;">HARGA</span>
                                        <span style="font-size: 16px; color: #009688; font-weight: bold;">JUMLAH</span>
                                        <br>
                                    </div>
                            </th><!-- <th>Jumlah</th> -->
                            <th style="text-align: right; color: black;"><b>AK</b><span
                                    style="color: black;"><b>SI</b></span><br>
            ...
            <div>
                <button type="button" class="w3-button w3-green" onclick="openBarangModal()"
            ...
            <?php if ($is_admin) { ?>
                ...
                <select class="w3-select w3-border w3-light-grey" name="id_toko" required>
                    ...
                    include 'koneksi.php';
                    $query = "SELECT IDTOKO, NAMATOKO FROM eat_and_go_toko";  // Mengambil semua toko
            ...
            <?php } else { ?>
                ...
                <?php
                // Mengambil IDTOKO dari URL jika tersedia
                $id_toko = isset($_GET['IDTOKO']) ? intval($_GET['IDTOKO']) : (isset($user_data['IDTOKO']) ? $user_data['IDTOKO'] : null);

                // Query untuk mengambil nama toko berdasarkan IDTOKO
                if ($id_toko) {
...
document.getElementById('jenispembayaran').addEventListener('change', function() {
    ...
    <small id="bayarError" class="text-danger" style="display: none;">JUMLAH BAYAR KURANG</small>
            ...
            <button type="button" class="w3-button w3-green w3-margin-top" id="payButton" onclick="openModal()" disabled
